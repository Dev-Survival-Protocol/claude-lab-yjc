...
| 구분                           | 로드 시점                   |
| ---------------------------- | ----------------------- |
| CLAUDE.md (`@path` 포함)       | 모든 세션 시작 시 항상 로드        |
| `.claude/rules/` (paths 미지정) | 모든 세션 시작 시 항상 로드        |
| `.claude/rules/` (paths 지정)  | 일치하는 파일을 열거나 작업할 때 로드   |
| 자동 메모리 (`MEMORY.md`)         | 모든 세션 시작 시 로드 (처음 200줄) |
| Skills                       | 스킬이 호출되는 시점에 로드         |
...
YAML frontmatter의 `paths` 필드로 특정 파일에만 규칙을 적용할 수 있다. `paths`를 지정한 규칙은 세션 시작 시 로드되지 않고, 일치하는 파일을 열거나 작업할 때만 로드된다.
